...
static inline int makeAxisInteger(const int& tick_count, const int& max) {
    if (!tick_count)
        return max;
    int tick_count_minus_one = tick_count - 1;
    int distance = max / tick_count_minus_one;
    return max % tick_count_minus_one ? distance * tick_count_minus_one + tick_count_minus_one
                                      : distance * tick_count_minus_one;
}

template<size_t Size>
QChart* lon::ChartsWidget::initLineChartSeries(const std::array<uint16_t, Size>& data_array,
                                               QString title /*= QString()*/) {
    // 直接使用 QChart，不需要 QtCharts:: 前缀
    ...
    // 修复：变量名和逻辑错误
    int max_value = *std::max_element(data_array.begin(), data_array.end());
    max_value = std::max(2, max_value);

    chart->createDefaultAxes();
    auto axesV = chart->axes(Qt::Vertical);
    auto* ver_axis = dynamic_cast<QValueAxis*>(!axesV.isEmpty() ? axesV.first() : nullptr);
    auto axesH = chart->axes(Qt::Horizontal);
    auto* hor_axis = dynamic_cast<QValueAxis*>(!axesH.isEmpty() ? axesH.first() : nullptr);

    ver_axis->setGridLineVisible(true);
    ver_axis->setTickCount(std::min(15, max_value + 1) + 1);
    ver_axis->setRange(0, makeAxisInteger(ver_axis->tickCount(), max_value));
    ver_axis->setLabelFormat("%d");

    hor_axis->setRange(0, std::max(Size - 1, static_cast<size_t>(1)));
    hor_axis->setGridLineVisible(true);
    hor_axis->setTickCount(Size);
...
QChart* lon::ChartsWidget::initPieChartSeries(
    ...
    chart->addSeries(series);
    chart->legend()->setVisible(false);  // 修复：补全缺失的代码
    return chart;  // 修复：补全返回语句
}

void lon::ChartsWidget::initDayFinishedLineChart() {
    finished_day_line_chart_p_ = initLineChartSeries(todaydata_p_->time_data_p);
