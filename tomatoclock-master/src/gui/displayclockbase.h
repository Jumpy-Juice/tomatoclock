...
protected:
    ...
    double currentPersent(TimerStatus const* timer_status) {
        const ClockTime* total_time = timer_status->currentStageTotalTime();
...
public:
    explicit DisplayClockBase(QWidget* parent = nullptr) : QWidget(parent) {
        working_foreground_brush_ = std::make_unique<QBrush>(QColor(255, 255, 255, 180));
        shortbreaking_foreground_brush_ = std::make_unique<QBrush>(QColor(30, 239, 61, 180));
        longbreaking_foreground_brush_ = std::make_unique<QBrush>(QColor(33, 252, 217, 180));
    }

    virtual ~DisplayClockBase() = default;
