...
private:
    ...
    QAction*       show_action_p_{nullptr};  // 新增：显示主窗口的菜单项
    QAction*       translate_action_p_{nullptr};  // 新增：翻译功能的菜单项
    QAction*       calculator_action_p_{nullptr};  // 新增：计算器功能的菜单项
    QWidgetAction* clock_widget_action_p_{nullptr};
    
    Translate*     translate_widget_p_{nullptr};  // 翻译功能窗口
    Calculator*    calculator_widget_p_{nullptr};  // 计算器功能窗口
private:
...
public slots:
    ...
    void saveSettingToFile(const ClockOptions& option);
    /// <summary>
    /// 系统托盘响应动作.
    /// </summary>
